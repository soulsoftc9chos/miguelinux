...
class ISAssetManager{
    ...
    func getAlbums(isPickVideo:Bool ,isPickerImage:Bool , completion:([ISAlbumModel])->()){
...
//        fetchOptions.predicate = NSPredicate(format: "mediaType = %d", PHAssetMediaType.image.rawValue)
        ...
        photoAlbums.enumerateObjects({ (collection, index, stop) in
            ...
            if (fetchResult.count>0){
                albums.append(self.modelWithFetchResult(result: fetchResult,name:collection.localizedTitle!,isPickVideo:isPickVideo,isPickerImage:isPickerImage))
            }
        })
        ...
        albums.sort { (alb1:ISAlbumModel, alb2:ISAlbumModel) -> Bool in
            return alb1.assets.count>alb2.assets.count
        }
        completion(albums)
    }
    
    func modelWithFetchResult(result:PHFetchResult<PHAsset>,name:String,isPickVideo:Bool,isPickerImage:Bool) -> ISAlbumModel {
        var assets = [ISAssetModel]()
    ...
    func getAlbumImage(album:ISAlbumModel,expectWidth:CGFloat,completion:@escaping ((UIImage?)->())){
