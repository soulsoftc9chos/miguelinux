...
class ISAssetCell: UICollectionViewCell{
    ...
    var asset:ISAssetModel?{
        didSet{
            ...
            selBtn.addTarget(self, action:#selector(setImgAct(sender:)), for: UIControlEvents.touchUpInside)
    ...
    var imgSled: Bool = false{
        didSet{
            if(imgSled){
                self.selImg.image = UIImage.loadBundleImage(bundle:IS_IMG_PICK_CONFIG.bundle,name:IS_IMG_PICK_CONFIG.selImg)
            }else {
                self.selImg.image = UIImage.loadBundleImage(bundle:IS_IMG_PICK_CONFIG.bundle,name:IS_IMG_PICK_CONFIG.unSelImg)
            }
        }
    }
    
    func setImgAct(sender:UIButton){
