...
class ISAlbumPickerController:UIViewController,UITableViewDelegate,UITableViewDataSource
{
    ...
    override func viewDidLoad() {
        ...
        self.loadAllAlbum()
        self.navigationItem.title = String.localsizeStringFrom(bundle:IS_IMG_PICK_CONFIG.bundle, forkey:"Photos")
    ...
    func cancelAct(sender:UIButton) -> Void{
        self.dismiss(animated: true, completion:nil )
    }
    func loadAllAlbum(){
            if ISAssetManager.shareInstance.authorized {
                 tableView.isHidden = false
                ISAssetManager.shareInstance.getAlbums(isPickVideo: true, isPickerImage: true) { (albums:[ISAlbumModel]) in
                    if albums.count>0{
                        self.albums = albums;
                        var counter = 0
                ...
                settingLabel.text = String(format:String.localsizeStringFrom(bundle: IS_IMG_PICK_CONFIG.bundle, forkey: "Allow %@ to access your album in \"Settings -> Privacy -> Photos\""),appName as! CVarArg)
...
//                settingLabel.trailingAnchor
                ...
                settingBtn.setTitle( String.localsizeStringFrom(bundle: IS_IMG_PICK_CONFIG.bundle, forkey: "Setting"), for: UIControlState.normal)
                settingBtn.setTitleColor(UIColor.blue, for: UIControlState.normal)
                settingBtn.addTarget(self, action: #selector(settingBtnAct(sender:)), for: UIControlEvents.touchUpInside)
    ...
    func settingBtnAct(sender:UIButton) -> Void{
    ...
    func tableView(_ tableView: UITableView, cellForRowAt indexPath: IndexPath) -> UITableViewCell {
        ...
        if let cell = cell {
            ...
            ISAssetManager.shareInstance.getAlbumImage(album: album,expectWidth:IS_IMG_PICK_CONFIG.expectImageWidth,completion: { (img:UIImage?) in
                if let img = img{
